...
class XLerobot(Robot):
    ...
    def _state_ft(self) -> dict[str, type]:
        return dict.fromkeys(
            (
                ...
                # MODIFICATION: Changed head_motor_1 to report velocity (.vel) instead of position (.pos)
                "head_motor_1.vel",
                "head_motor_2.pos",
                "x.vel",
                "y.vel",
    ...
    def calibrate(self) -> None:
        ...
        # MODIFICATION START: Treat head_motor_1 as a full-turn motor.
        # This skips manual range recording for head_motor_1 (ID 7 on bus 1)
        # and sets its range to the maximum (0-4095), allowing for full rotation.
        ## calib left motors
        ...
        full_turn_motors_left = ["head_motor_1"]
        motors_to_home_left = [motor for motor in left_motors if motor not in full_turn_motors_left]

        input(
            f"Move all left arm and head joints except '{full_turn_motors_left}' to the middle of their range of motion and press ENTER...."
        )
        homing_offsets = self.bus1.set_half_turn_homings(motors_to_home_left)
        homing_offsets.update(dict.fromkeys(full_turn_motors_left, 0))
        homing_offsets.update(dict.fromkeys(self.right_arm_motors + self.base_motors, 0))

        unknown_range_motors_left = [motor for motor in left_motors if motor not in full_turn_motors_left]
        print(
            f"Move all left arm and head joints except '{full_turn_motors_left}' sequentially through their "
            "entire ranges of motion.\nRecording positions. Press ENTER to stop..."
        )
        range_mins, range_maxes = self.bus1.record_ranges_of_motion(unknown_range_motors_left)

        for name in full_turn_motors_left:
            range_mins[name] = 0
    ...
    def configure(self):
        ...
        # MODIFICATION: Set head_motor_1 to VELOCITY mode for continuous rotation.
        for name in self.head_motors:
            if name == "head_motor_1":
    ...
    def get_observation(self) -> dict[str, Any]:
        ...
        # MODIFICATION: Read velocity for head_motor_1 and position for other head motors
        head_motors_pos_names = [m for m in self.head_motors if m != "head_motor_1"]
        head_motors_vel_names = ["head_motor_1"]

        head_pos_raw = self.bus1.sync_read("Present_Position", head_motors_pos_names)
        head_vel_raw = self.bus1.sync_read("Present_Velocity", head_motors_vel_names)
        ...
        # Combine head motor states
        head_pos_state = {f"{k}.pos": v for k, v in head_pos_raw.items()}
        head_vel_state = {f"{k}.vel": v for k, v in head_vel_raw.items()}
        head_state = {**head_pos_state, **head_vel_state}
    ...
    def send_action(self, action: dict[str, Any]) -> dict[str, Any]:
        ...
        # MODIFICATION: Separate head motor commands into position and velocity
        head_pos = {k: v for k, v in action.items() if k.startswith("head_") and k.endswith(".pos")}
        head_vel = {k: v for k, v in action.items() if k.startswith("head_") and k.endswith(".vel")}

        base_goal_vel = {k: v for k, v in action.items() if k.endswith(".vel") and not k.startswith("head_")}
        ...
        if self.config.max_relative_target is not None:
            # Read present positions for left arm, right arm, and head motors in position mode
            ...
            head_motors_pos_names = [m for m in self.head_motors if m != "head_motor_1"]
            present_pos_head = self.bus1.sync_read("Present_Position", head_motors_pos_names)

            # Combine all present positions for safety check
            present_pos = {**present_pos_left, **present_pos_right, **present_pos_head}

            # Create a dictionary of goal and present positions for position-controlled motors
            goal_present_pos = {
                key: (g_pos, present_pos.get(key))
                for key, g_pos in chain(left_arm_pos.items(), right_arm_pos.items(), head_pos.items())
                if present_pos.get(key) is not None
            }
        ...
        # Prepare raw commands for writing
        ...
        head_vel_raw = {k.replace(".vel", ""): v for k, v in head_vel.items()}
        ...
        if head_vel_raw:
            self.bus1.sync_write("Goal_Velocity", head_vel_raw)
    ...
    def disconnect(self):
        ...
        self.stop_base()
        # Also stop head motor if it's in velocity mode
        self.bus1.sync_write("Goal_Velocity", {"head_motor_1": 0})
